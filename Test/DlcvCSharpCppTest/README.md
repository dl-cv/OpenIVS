...
| 按钮 | 行为 |
|---|---|
| 加载C#模型 | 使用路径框和设备编号创建 C# `Model`；路径为空时打开文件选择窗口 |
...
- `designer-test` 仅构造和释放未显示的窗体，检查六按钮，并检查进程未加载 C++/CLI 桥接或原生推理模块。
- `ui-test` 无 `--model` 时检查六按钮及可用状态、空模型操作、重复释放、无效模型、禁止的模型格式和负索引。
- 指定模型时额外验证两侧索引一致、两侧信息、两种释放顺序、释放后再次转换、最终索引失效及窗口对象释放。
- 测试仅创建未显示的 WinForms 对象，不执行桌面自动化、键鼠模拟、按钮点击或窗口控制；不验证人工文件选择和界面绘制。
