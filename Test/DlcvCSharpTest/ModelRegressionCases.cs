...
namespace DlcvCSharpTest
{
    ...
    internal static class ModelRegressionCases
    {
        ...
        // 类别、分数、bbox 等基准来自 dlcv_infer 的 shared_model_regression_cases.json。
        // 掩码尺寸与像素统计按 C# API 缩放后的局部 Mat 记录；有掩码时 Area 采用独立核验的非零像素数。
        public static readonly List<ModelRegressionCase> Cases = new List<ModelRegressionCase>
        {
            Case("测试无监督 DVT", "测试-无监督检测_PLUS_s.dvt", "1786969663716.jpg",
                Sample(Result(0, "NG", 1.0, B(0.0, 13.0, 438.0, 611.0), 249853.0, -100.0, true, true, false, M(438, 611, 249853)))),
            Case("猫狗分类 Sentinel DVT", "猫狗-分类_PLUS_s.dvt", "猫狗-狗.jpg",
            ...
            Case("气球语义分割 DVT", "气球-语义分割_PLUS_s.dvt", "气球.jpg",
                Sample(Result(0, "气球", 1.0, B(68.5714340209961, 54.85714340209961, 498.2857360839844, 579.4285888671875), 222774.0, -100.0, true, true, false, M(498, 579, 222774)))),
            Case("手机屏幕实例分割 DVT", "手机屏幕-实例分割_PLUS_s.dvt", "手机屏幕.jpg",
            ...
            Case("引脚定位目标检测 DVT", "引脚定位-目标检测_PLUS_s.dvt", "引脚定位-目标检测.jpg",
                Sample(
            ...
            Case("AOI 旋转框检测 DVT", "AOI-旋转框检测_PLUS_s.dvt", "AOI-测试.jpg",
                Sample(
                    ...
                    Result(2, "三极管", 0.95947265625, B(523.75, 556.875, 550.3125, 507.8125), 279772.0, 0.802734375, true, false, true, null),
                    Result(2, "三极管", 0.95166015625, B(220.9375, 1198.75, 629.375, 508.75), 320194.0, -0.798828125, true, false, true, null))),
            Case("OCR DVT", "文字-OCR_PLUS_s.dvt", "OCR-472.jpg",
