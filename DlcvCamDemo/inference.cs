...
namespace DlcvCamDemo
{
    ...
    public class Inference
    {
        ...
        public void SetApiModelPath(string path)
        {
            ...
            {
                throw new ArgumentException("API模型路径不能为空", nameof(path));
            }

            _apiModelPath = path;
            _model = null;
            SetInferenceMode(apiMode: true);
        }
        ...
        public void SetModel(Model model)
        {
            ...
            _model = model;
            _apiModelPath = null;
            SetInferenceMode(apiMode: false);
        }

        /// <summary>
        /// 设置推理模式，确保 DLL/API 标志位互斥
        /// </summary>
        /// <param name="apiMode">true 为 API 模式，false 为 DLL 模式</param>
        private void SetInferenceMode(bool apiMode)
        {
            _apiFlag = apiMode;
            _dllFlag = !apiMode;
        }
        ...
        public dynamic ProcessInference(Bitmap bitmap, string filePath)
        {
            ...
            try
            {
                if (_dllFlag)
                {
                    if (_model == null)
                    {
                        throw new InvalidOperationException("当前为 DLL 模式，但模型尚未初始化");
                    }
                ...
                {
                    if (string.IsNullOrWhiteSpace(_apiModelPath))
                    {
                        throw new InvalidOperationException("当前为 API 模式，但模型路径为空");
                    }
