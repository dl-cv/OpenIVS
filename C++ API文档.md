...
### 13.1 `dlcv_infer_c_dll` 与 `dlcv_infer` C API 对照
...
| --- | --- | --- | --- |
| JSON 模型加载、释放、信息、推理 | `dlcv_load_model`、`dlcv_free_model`、`dlcv_get_model_info`、`dlcv_infer` | 无 JSON 返回入口 | 未覆盖 |
| JSON 结果释放 | `dlcv_free_model_result`、`dlcv_free_result` | 无对应入口 | 未覆盖 |
| 释放全部模型 | `dlcv_free_all_models` | 无对应入口 | 未覆盖 |
...
完整性结论分为两部分：若范围是 `dlcv_infer` 的全部 23 个导出函数，`dlcv_infer_c_dll` 并不完整；若范围是加载、释放、结构化推理、结构化结果释放四项模型核心能力，则已完整覆盖，并增加参数 JSON、加载错误读取和流程归档模型支持。
...
两侧使用的 `dlcv_data_type_c.h` 当前文件内容相同。在 MSVC v143、Windows x64、默认 packing 下，结构大小也相同。

| 结构 | 字段或大小 | 两侧含义 | 一致性 |
| --- | --- | --- | --- |
| `DlcvCImage` | `data_ptr`、`height`、`width`、`channel`；24 字节 | 图像数据地址及尺寸 | 一致 |
| `DlcvCImageList` | `images`、`n`；16 字节 | 图像数组及批量数量 | 一致 |
| `DlcvCMask` | `mask_ptr`、`height`、`width`；16 字节 | mask 数据地址及尺寸 | 一致 |
| `DlcvCObjectResult` | 96 字节 | 单个目标的类别、分数、框、mask、角度和均值 | 一致 |
| `DlcvCSampleResult` | 16 字节 | 单张图像的目标数组及数量 | 一致 |
| `DlcvCResult` | 32 字节 | 调用状态、消息、样本数组及数量 | 一致 |

两侧结构化推理都把图像解释为连续、紧密排列的 8 位交错通道数据，不接管输入内存。结构中没有 `dtype` 和行跨度字段，16 位、浮点或带行填充的图像需先转换。颜色次序不由接口固定，按模型预处理要求提供；当前专项测试使用 RGB 数据。

`dlcv_infer_c` 没有参数 JSON，使用模型默认推理参数。`dlcv_infer_cpp_infer_with_params_c` 可额外传入 JSON 对象，空字符串或空指针与默认推理入口等效。
