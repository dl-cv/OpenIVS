...
- `index` 使用加载返回的原值：非负数为普通模型，小于 `-1` 为流程 handle；`-1` 或索引不可用时抛出异常。
...
- 按 index 释放时，参数格式无效或编号为 `-1` 或超出有符号 `int` 范围仍可返回参数错误；参数是有效编号时，即使编号已不存在或底层释放返回错误，也返回成功并完成本地清理，重复释放同样成功。底层错误最多附在日志或 `message` 的错误详情中，不保留等待重试状态。无参数的 `FreeModel()` 在对象已释放时直接成功。
