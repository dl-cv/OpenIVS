...
- **下拉框**：设备选择；计算均值可选 `默认`、`是`、`否`，初始值为 `默认`。`默认` 不发送 `calc_mean`，`是` 发送 `true`，`否` 发送 `false`。
- **Label**：`选择显卡`、`线程数`、`batch_size`、`threshold`、`计算均值`。
- **数值输入**：
  - 线程数（1-32，默认1）。
...
- **文本输出**
  ...
  - 右键菜单采用 WinForms 原生菜单，包含撤销、剪切、复制、粘贴、删除、全选和自动换行。
  - 撤销、剪切、复制、粘贴、删除、全选分别显示对应的常用快捷键；没有可操作内容时，相关菜单项不可用。
  - `自动换行`为可勾选菜单项，勾选状态与 `richTextBox1.WordWrap` 保持一致。
...
- **输入**：
  ...
  - 参数：UI 设置的 Batch Size、Threshold、计算均值状态，强制 `with_mask=true`；计算均值为 `默认` 时省略 `calc_mean`，`是` 与 `否` 写入对应布尔值。
...
- 处理流程：
  ...
  - `with_mask = true`
  - `comboBox_calc_mean` 选择 `默认` 时省略 `calc_mean`；选择 `是` 与 `否` 时分别写入 `true` 与 `false`。
