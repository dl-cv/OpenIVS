...
- **数值输入**：
  ...
  - Threshold（0.00-1.00，步进0.05，默认0.50）。当已加载模型、已选择图片且没有运行压力/一致性测试时，调整该值会立即重新执行一次“单次推理”并刷新文本与图像结果。
- **复选框**：`RPC模式`（默认不勾选）。

- **文本输出**
  - `richTextBox1`：WinForms 只读多行 TextBox，用于显示错误、模型信息、推理结果文本和测试统计信息。
  - 右键菜单采用原生 ContextMenuStrip，包含撤销、剪切、复制、粘贴、删除、全选和自动换行。
  - 只读状态下，修改内容的菜单禁用；复制和全选仍可使用。
  - `自动换行`为可勾选菜单项；勾选时启用文本换行并关闭水平滚动条，取消勾选时恢复不换行和水平滚动条。
...
- 置 `TopMost=false`
- `加载模型`按钮从窗口打开开始即可使用，不等待设备信息或环境检查；设备列表尚未完成时加载模型使用 CPU（device_id=-1）。
- 启动时不自动执行环境检查；环境信息仅在点击`检查环境`后读取。
- 启动后台线程执行“设备信息获取”，完成后更新 UI：
  - 清空设备列表
  - 依次添加 `OpenVINO`（device_id=-2）、`CPU`（device_id=-1），再追加 GPU。
  - 若 GPU 枚举成功：追加每张 GPU 的 `device_name`，映射为 device_id=0,1,2…
  - 默认选择：
    - 有 GPU：选择第一个 GPU（下拉索引 2）
    - 无 GPU：选择 CPU（下拉索引 1）
...
- **前置条件**：无（可在未选设备时使用；默认CPU=-1），环境检查未执行、正在执行或失败均不影响选择和加载模型。
- **文件选择对话框**：
  - 标题：`选择模型`
  - 过滤器：
...
- 若测试运行中，先停止测试
- 释放当前模型并清理本地引用
- 按 index 释放时，参数格式无效或编号为 `-1` 或超出有符号 `int` 范围仍可返回参数错误；参数是有效编号时，即使编号已不存在或底层释放返回错误，也返回成功并完成本地清理，重复释放同样成功。底层错误最多附在日志或 `message` 的错误详情中，不保留等待重试状态。界面对象已释放后再次释放也显示成功。
...
- 按 index 释放时，参数格式无效或编号为 `-1` 或超出有符号 `int` 范围仍可返回参数错误；参数是有效编号时，即使编号已不存在或底层释放返回错误，也返回成功并完成本地清理，重复释放同样成功。底层错误最多附在日志或 `message` 的错误详情中，不保留等待重试状态。`Utils.FreeAllModels()` 重复调用显示成功。
...
- 环境检查仅由本按钮触发，窗口启动和模型加载不会自动执行，也不等待检查结果。
...
- **环境检查**
  - 窗口启动和模型加载不会自动执行环境检查，`加载模型`按钮不等待环境检查。
