...
void ImageViewerWidget::setImageAndResults(const cv::Mat& bgrImage, const std::vector<DisplayObjectResult>& results) {
    setImage(bgrImage);
    setResults(results);
}
...
void ImageViewerWidget::setResults(const std::vector<DisplayObjectResult>& results) {
    results_ = results;
    update();
}
...
QImage ImageViewerWidget::createMaskOverlayImage(const cv::Mat& mask) {
    ...
    // 与 C# Demo 的 PixelFormat.Format32bppArgb 保持一致，使用非预乘 alpha，避免不同平台下的混合差异。
...
void ImageViewerWidget::drawResults(QPainter& painter, QString& statusText, bool& shouldDrawStatus) const {
    ...
    auto drawMask = [&](const DisplayObjectResult& obj, const QRectF& targetRect, bool rotateWithBbox) {
    ...
    for (const DisplayObjectResult& obj : results_) {
