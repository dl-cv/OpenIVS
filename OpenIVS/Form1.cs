    ...
    public partial class Form1 : Form
    {
        ...
        private string PerformInference(Bitmap image)
        {
            try
            {
                ...
                Cv2.CvtColor(mat, imageBgr, ColorConversionCodes.BGR2RGB);
                
                // 创建批处理列表
                var imageList = new List<Mat> { imageBgr };
        ...
        private void UpdateDeviceStatus(string status)
        {
            if (InvokeRequired)
            {
                BeginInvoke(new Action<string>(UpdateDeviceStatus), status);
                return;
            }
        ...
        private void UpdateCameraStatus(string status)
        {
            if (InvokeRequired)
            {
                BeginInvoke(new Action<string>(UpdateCameraStatus), status);
                return;
            }
        ...
        private void UpdateModelStatus(string status)
        {
            if (InvokeRequired)
            {
                BeginInvoke(new Action<string>(UpdateModelStatus), status);
                return;
            }
        ...
        private void UpdatePositionDisplay(float position)
        {
            if (InvokeRequired)
            {
                BeginInvoke(new Action<float>(UpdatePositionDisplay), position);
                return;
            }
        ...
        private void UpdateDetectionResult(string result)
        {
            if (InvokeRequired)
            {
                BeginInvoke(new Action<string>(UpdateDetectionResult), result);
                return;
            }
