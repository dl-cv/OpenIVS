...
- 代码里的 `imageRgb`、`fullImageRgb` 等命名沿用旧字段名；当输入是灰度图时，这些变量实际承载的是“送入模型的图像”，不强制为三通道。
...
- 不展示模型1中间框。
- 不展示中间裁图。
- 标签显示沿用 `DLCV.ImageViewer`：默认格式为 `{category_name} {score:F2}`，可按 `C` 在 `类别+分数`、`仅类别`、`不显示` 三种模式间循环；`+` / `-` / `0` 与 `Ctrl + 滚轮` 调整标签字体倍率。
...
- 模型字段：`model1`、`model2`
- 状态字段：`imagePath`、`isInferenceRunning`；速度测试相关字段（如 `PressureTestRunner`、定时器、测速用推理输入图、启动时冻结的模型2线程数等）
