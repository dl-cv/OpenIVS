...
程序直接复制 `DlcvDemo2` 项目并改写，不抽公共基类，不抽公共工具类，不做额外共享封装。与当前业务强相关的逻辑全部直接写在 `DlcvDemo3/Form1.cs` 中，通过私有字段、私有方法和私有内部类完成。
...
以下代码块以 `C#` 语法形式概括 `RunPipeline` 的主干流程：
...
private PipelineRunResult RunPipeline(Mat fullImageRgb, IProgress<InferenceProgressInfo> progress = null)
{
    ...
    {
        if (TryClampObjectToImage(obj, fullImageRgb.Width, fullImageRgb.Height, out var clampedStage1))
        {
            model1Objects.Add(clampedStage1);
        }
    }
    ...
    foreach (var obj in model1Objects)
    {
        ...
        CenteredCropContext ctx = BuildCenteredCropContext(
            fullImageRgb,
            center,
            FixedCropWidth,
            FixedCropHeight);
        ...
        if (ctx.IsValid)
        {
            cropContexts.Add(ctx);
        }
        else
        {
            runResult.Logs.Add($"跳过目标[{obj.CategoryName}]：{ctx.InvalidReason}");
        }
    }
    ...
    int processed = 0;
    foreach (var chunk in SplitIntoChunks(cropContexts, batchLimit))
    {
        ReportInferenceProgress(progress, 30 + (int)Math.Round(55.0 * processed / Math.Max(1, cropContexts.Count)), $"模型2 batch 推理 {processed}/{cropContexts.Count}");

        List<Mat> mats = chunk.Select(x => x.CropRgb).ToList();
        CSharpResult batchResult = model2.InferBatch(mats, inferParams);
        ...
        for (int i = 0; i < chunk.Count; i++)
        {
            ...
            foreach (var localObj in localObjects)
            {
                ...
                {
                    if (TryClampObjectToImage(mapped, fullImageRgb.Width, fullImageRgb.Height, out var clamped))
                    {
