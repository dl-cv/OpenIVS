...
ZIP 只含 `SentinelManager.exe`、`README.md`，以及输出目录存在时的 `SentinelManager.exe.config`。不包含测试程序、PDB、源码、截图、JSON 结果、外部 DLL 或授权文件。不调用根目录 wheel、签名、上传流程，不修改密钥相关实现。
...
只覆盖清单内的同名文件，其他来源文件保留不动。安装包不含配置而安装目录已有同名配置时，安装停止，不删除现有配置。安装前应关闭正在运行的组件，文件被占用或权限不足时不得视为安装完成。
