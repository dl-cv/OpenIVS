...
两套控制台程序均支持以下子命令，不依赖模型文件：

- `mask-area-selftest`：校验结构化与 JSON 输出的掩码面积，包括缩放、非整数 bbox、尺寸不变、全零掩码、无掩码、稀疏掩码的最近邻插值，以及滑窗掩码合并。固定检查 4×4 全白掩码缩放为 2×2 时面积为 4，并检查输入未被改写。
- `region-mask-selftest`：校验 IOS/IOU、阈值、区域与目标掩码、仿射映射及原图编号隔离。

C# 的 `all-tests` 已包含这两项；C++ 面积测试另保留原始掩码尺寸解析的专项检查。两项自测均以退出码 0 表示通过，非零表示失败。
...
测试程序在单个进程内依次执行自测和固定模型回归用例，完整清单与顺序以 `Program.cs` 中的 `RunAllTests` 为准。完整清单执行结束后再返回，任一测试失败时返回 `1`，参数或日志路径无效时返回 `2`。日常完整验证只需启动脚本一次，无须分别调用各个 `*-selftest` 子命令。
