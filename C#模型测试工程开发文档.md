...
- `DlcvCSharpTest.exe shared-index-review-selftest` 检查以下事实：
  ...
  - 恢复不按模型/流程类型、编号数值或 `bit8` 推导所属 DLL；只枚举进程内实际已加载且具备类型查询导出的目标 DLL，不加载其他 infer DLL，由各 DLL 查询模型编号，并由 OpenIVS 流程表查询流程编号。
...
- `DlcvCSharpTest.exe shared-index-csharp-selftest` 依次执行以下检查：
  ...
  - 每次恢复时查询进程内实际已加载的目标 DLL；普通模型按已加载 DLL 查询绑定，流程额外要求通用编号分配能力及 OpenIVS 流程记录，歧义和异常均拒绝恢复。
...
C# 经 flow/SharedFlowRegistry.cs 访问既有 dlcv_infer_cpp.dll 中的流程记录；底层推理模块只保存模型并提供通用编号分配。流程查询返回 pipeline 和子模型绑定，恢复后两种语言分别持有自己的执行对象。子模型通过普通绑定接口复用，流程信息字符串由 OpenIVS 原生模块释放。工程引用会构建原生包装并复制其运行依赖，单独构建 C# Demo 也包含所需 DLL。
