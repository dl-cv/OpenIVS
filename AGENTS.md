...
| 能力 | 类/函数 | 关键接口 |
|------|---------|----------|
| 普通模型 | `DlcvModules.Model` | 构造（`string modelPath, int deviceId`）、`Infer()`、`InferBatch()`、`InferOneOutJson()`、`GetModelInfo()`、`Dispose()`、`Benchmark()` |
...
| 推理计时 | `DlcvModules.InferTiming` | `GetLastSdkMs()`、`GetLastFlowMs()`、`GetLastNodeTimings()` |
