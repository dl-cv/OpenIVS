...
普通模型索引由底层 `dlcv_infer` 加载接口返回。流程由 C# 或 C++ 完成解析和子模型加载，再向底层注册流程 JSON，并使用底层返回的流程索引。每段索引独立递增，不复用已释放索引。
...
- **功能**：通过 C API 加载 `.dvt` 与 `.dvst`，读取图片转为 RGB，执行结构化推理，并检查同一模型句柄的多线程结果一致性
- **验证输入**（固定写死在源码中）：
  - 普通模型：`Y:\测试模型\猫狗-分类_120_50_s.dvt`
  - 普通模型图片：`Y:\测试模型\猫狗-狗.jpg`
  - 流程模型：`Y:\测试模型\AOI_120_50_s.dvst`
  - 流程模型图片：`Y:\测试模型\AOI-1.jpg`
- **期望输出**：
  - `.dvt` 基准结果与 4 线程共 40 次结果一致
  - `.dvst` 基准结果与 4 线程共 40 次结果一致
  - 全新 `.dvst` 实例首次执行 4 线程共 40 次推理时结果一致且无访问异常
- **通过标准**：全部检查通过时打印 `Test PASSED` 并返回 0；推理错误或结果不一致时打印 `Test FAILED` 并返回 1
