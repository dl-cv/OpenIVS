...
- Sentinel 管理工具仅保留独立 C# 工程与测试，使用 Visual Studio 或仓库共享 `build.py` 进行日常项目编译，不提供独立打包、安装脚本。
...
- C# GUI 验证只使用 `DlcvDemo` 的 `ui-test` 命令行入口并固定 `--interactive-dialogs false`：程序自动加载参数中的模型与图片，完成或失败后自动关闭窗口，以进程退出码与 `--output` 的无 BOM UTF-8 JSON 结果文件为判断依据，输出写入系统临时目录；禁止通过桌面自动化、鼠标键盘或窗口控制验证，也不以无参数 GUI 启动代替测试。可选 `--language zh-CN|en-US` 指定界面语言（默认中文，不持久化）。可选 `--result-view summary|json` 指定结果区显示汇总或 JSON（默认汇总，不持久化，不重新推理）。
...
- **测试范围**：模型加载成功/失败、推理成功/失败、推理结果类别列表、3 秒平均推理速度、Batch 推理速度、内存泄露专项（统一回归仅对 1 个实例分割模型执行加载/释放循环 10 次 + 推理 3 秒内存增量；`model-load-free-memory-selftest` 可指定循环次数并输出分段内存与活动模型数；`model-load-free-stage-selftest` 分别输出程序启动、每次加载完成和释放完成的内存）
- **默认模型目录**：`Y:\测试模型`
