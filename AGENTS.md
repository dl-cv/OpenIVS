...
**实现位置**：`dlcv_infer_cpp_dll/dlcv_infer_c_api.h` + `dlcv_infer_c_api.cpp`，基于 `dlcv_infer::Model` 封装，零侵入现有 C++ 源码。
...
- **开分支**：任何改动必须先基于 master 创建功能分支，禁止直接在 master 上提交。
- **必须使用 `gh` CLI**：分支创建、PR 创建、PR 合并全部通过 `gh` 命令完成，禁止在本地直接用 `git merge` 合入 master。
- **禁止本地 merge master**：即使本地 master 分支，也禁止执行 `git merge` 将功能分支直接合入；必须通过 GitHub Pull Request 流程合并。
- **检查工作正确**：提交前必须验证改动正确性（运行测试、检查编码、确认无乱码）。
- **PR 标题使用中文**：格式为 <类型>: <简述>，例如 docs: 修复 AGENTS.md 编码问题。
- **PR 内容描述工作**：正文必须描述本次工作的具体内容，不得为空。
- **压缩提交合并**：合并时必须使用 **Squash Merge**，将分支上的多个提交压缩为单个提交后合并。命令示例：`gh pr merge <pr-number> --squash --delete-branch`。
