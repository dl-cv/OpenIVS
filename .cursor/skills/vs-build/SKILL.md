---
name: vs-build
description: 使用项目级 skill 内置的本地 Python 构建脚本执行编译、构建、重建与发布前构建验证。用户提到编译、构建、build、rebuild、重新编译、解决方案、项目、sln、csproj、vcxproj、发布前验证时使用。禁止切换到 dotnet、msbuild、powershell 或其他直接编译命令。
---
...
- 发布前构建验证
- 构建某个解决方案或某个 C# / C++ 项目
- 提到 `.sln`、`.csproj`、`.vcxproj`、解决方案、项目路径
...
2. 用户明确给出 `.csproj` 或 `.vcxproj` 路径：构建该项目。
3. 用户只说“编译这个项目”，但上下文已明确某个 C# 或 C++ 项目：解析到对应 `.csproj` 或 `.vcxproj` 后构建该项目。
4. 用户只说“编译一下”“帮我构建”而未限定范围：优先使用仓库默认解决方案。
5. 若请求针对的不是 `.csproj` 或 `.vcxproj`，不要猜测其他编译命令；优先转为包含它的解决方案构建，或先向用户确认范围。

## 执行流程

### 1. 识别范围

- 先判断是“整个解决方案”还是“单个 C# / C++ 项目”。
...
- 单个 C++ 项目编译：`<target-path>` 使用 `.vcxproj`
- 重建：`--target Rebuild`
- 清理：`--target Clean`
