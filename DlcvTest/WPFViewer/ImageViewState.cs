...
{
    /// <summary>
    /// 共享视图状态：用于两个 Viewer 同步缩放/平移�?    /// 坐标约定：屏幕坐�?= Scale * 图像像素坐标 + Offset�?    /// </summary>
    public sealed class ImageViewState : INotifyPropertyChanged
    {
        ...
        public void SetView(double scale, double offsetX, double offsetY)
        {
            ...
            // 仅通知真正变化的属性，避免高频交互时触发多次 UI 更新
            if (scaleChanged) OnPropertyChanged(nameof(Scale));
